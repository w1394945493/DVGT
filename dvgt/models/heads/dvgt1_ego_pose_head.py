...
class DVGT1EgoPoseHead(nn.Module):
    """
    ...
    def __init__(
        ...
        self.ego_pose_token_index = ego_pose_token_index
    ...
    def forward(self, aggregated_tokens_list: list, num_iterations: int = 4) -> list:
        """
        Forward pass to predict ego pose parameters.
        ...
        Returns:
            list: A list of predicted ego pose encodings (post-activation) from each iteration.
        """
    ...
    def trunk_fn(self, pose_tokens: torch.Tensor, num_iterations: int) -> list:
        ...
        Args:
            pose_tokens (torch.Tensor): Normalized ego pose tokens with shape [B, T, C].
            num_iterations (int): Number of refinement iterations.

        Returns:
            list: List of activated ego pose encodings from each iteration.
        """
        B, T, C = pose_tokens.shape
        pred_pose_enc = None
        pred_pose_enc_list = []
